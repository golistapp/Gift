(async function() {
    ...
    function populateFormForEdit() {
        ...
        // 🔴 NAYA CODE: Passcode pre-fill karne ke liye
        if (md.passcode) {
            const prefix = state.memoryId + "-";
            // Prefix (GX-02-) ko hatakar sirf number input mein daalenge
            document.getElementById('passcode-input').value = md.passcode.startsWith(prefix) ? md.passcode.substring(prefix.length) : md.passcode;
        }
    ...
    async function compressImage(file) {
     ...
     async function uploadToImageKit(file, fileName) {
        // 1. Backend se secure parameters mangao (Private Key ab backend me hai)
        const authRes = await fetch("/api/upload-auth");
        if (!authRes.ok) throw new Error("Could not fetch auth parameters");
        ...
        // Secure auth data ImageKit ko bhejo
        formData.append("publicKey", "public_mOww3EhDXvtS8/35MxbmdZz0lWs="); // Tumhari public key
        formData.append("signature", authData.signature);
        formData.append("expire", authData.expire);
        formData.append("token", authData.token);

        // 3. ImageKit par secure upload
    ...
    if (previewBtn) {
        previewBtn.addEventListener('click', () => {
            ...
            const previewGiftData = {
                status: "locked", // 🔴 FAKE LOCK taaki viewer page isko block na kare
                is_enabled: true,
                occasion: occasion,
                girlfriend_name: gfName,
                message_text: finalPreviewMessage, 
            ...
            // 🔴 NAYA LOGIC: Data ko browser ki memory mein daalo taaki naya tab usko padh sake
            localStorage.setItem('gx_preview_data', JSON.stringify(previewGiftData));
